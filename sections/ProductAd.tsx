    ...
    return (
        <div class="w-11/12 max-w-7xl mx-auto flex my-8 flex-col justify-center items-center lg:flex-row">
            <div class="w-full max-w-sm relative">
                ...
                <button class="block lg:hidden bg-red-600 rounded-3xl px-6 py-3 absolute top-1 right-3 z-10">Save</button>
            </div>
            <div class="w-full flex flex-col justify-between">
                <div>
                    <div class="flex justify-between">
                        <h3 class="text-lg font-bold">{product.title}</h3>
                        <button class="hidden lg:block bg-red-600 rounded-3xl px-6 py-3">Save</button>
                    </div>
                    <p>{adDescription ?? product.description}</p>
                </div>
                <div class="flex flex-col lg:items-end items-center">
                    <p class="text-xl font-bold text-lime-600 mb-6">{formatPrice(product.price)}</p>
